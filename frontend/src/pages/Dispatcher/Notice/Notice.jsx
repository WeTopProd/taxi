...
const Notice = () => {
  ...
  return (
    <>
      <button
        className={cx(styles.btn, styles.notice)}
      >
        <svg className={cx(styles.notice_label, notice ? styles.notice_label_alert : '')}
        ...
        {
          notice ? <sub className={styles.notice_count}>{notice}</sub> : ''
        }
      </button>
    </>
  )
